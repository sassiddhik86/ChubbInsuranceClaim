...
namespace ChubbInsuranceClaim.src.Application.Services
{
    public class DashboardService : IDashboardService
    {
        ...
        public async Task<DashboardResponse> GetCustomerDashboardAsync(int customerId)
        {
        ...
        public async Task<DashboardResponse> GetOfficerDashboardAsync(int officerId)
        {
            var claims = await _unitOfWork.Claims.GetAssignedClaimsAsync(officerId);
        ...
        private static DashboardResponse BuildDashboard(List<InsuranceClaim> claims)
        {
            ...
            return new DashboardResponse
            {
                ...
                ApprovedClaims = claims.Count(x => x.Status == ClaimStatus.Approved),
                RejectedClaims = claims.Count(x => x.Status == ClaimStatus.Rejected),
                SettledClaims = claims.Count(x => x.Status == ClaimStatus.Settled)
            };
        }
    }
}
